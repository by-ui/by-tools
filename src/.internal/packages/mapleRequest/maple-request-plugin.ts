...
import { MapleRequestConfig } from './index';
...
const MapleRequestPlugin: PluginObject<MapleRequestConfig> = {
    install(Vue, options) {
         ...
         */
        Vue.prototype.$request = function (name: string, data: { [key: string]: any }, loading: boolean = false, queue: string | boolean = false) {
            /** api列表中对应的配置项 */
            let api = apis[name];
            ...
            if (api) {
                ...
                if (api.textMark || api.openMark) {
                    ...
                    return new Promise((resolve, reject) => {
                        ...
                        // mock
                        if (options && options.create && options.create.mock && options.create.mock.config.indexOf(name) >= 0) {
                            axiosConfig.baseURL = options.create.mock.url;
                        }
